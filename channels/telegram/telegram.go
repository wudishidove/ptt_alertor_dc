...
func init() {
	log.Infof("讀取 TELEGRAM_TOKEN: '%s'", token)
	if token == "" || token == "0" || token == "YOUR_TELEGRAM_TOKEN" {
		log.Info("Telegram Bot 已禁用，因為 token 為空、為 0 或使用了預設值")
		return
	}

	log.Info("條件不滿足，嘗試初始化 Telegram Bot...")
	bot, err = tgbotapi.NewBotAPI(token)
	if err != nil {
		// 在記錄 Fatal 錯誤之前，先記錄詳細的 token 值
		log.Errorf("使用 Token '%s' 初始化 Telegram Bot 失敗", token)
